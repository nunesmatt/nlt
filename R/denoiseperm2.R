`denoiseperm2` <-function (x, f, pred=1, neigh=1, int=TRUE, clo=FALSE, keep=2, rule = "median", index = 1:length(x), verbose = FALSE, 
...
out <- fwtnpperm2(x, f, LocalPred=pred, neigh=neigh, int=int,clo= clo,nkeep=keep,mod=per,varonly=TRUE)
newcoeff <- out$coeff
